...
def _get_tree_traversal(root_node, start_idx=0, max_degree=None):
    ...
    for layer in reversed(layers):
        for node in layer:
            ...
            child_indexes = [(child.idx if child else -1)
                             for child in node.children]  # idx of child node
            if max_degree is not None:
                child_indexes.extend([-1] * (max_degree - len(child_indexes)))
            assert not any(idx is None for idx in child_indexes)

            node.idx = idx
            tree.append(child_indexes + [node.idx])
...
class RvNN(object):
    ...
    def __init__(self, word_dim, hidden_dim=5, n_class=4,
    ...
    def create_output_fn(self):
        self.W_out = theano.shared(self.init_matrix([self.n_class, self.hidden_dim]))
        self.b_out = theano.shared(self.init_vector([self.n_class]))
    ...
    def create_recursive_unit(self):
        ...
        self.params.extend(
            [self.E, self.W_z, self.U_z, self.b_z, self.W_r, self.U_r, self.b_r, self.W_h, self.U_h, self.b_h])
    ...
    def gradient_descent(self, loss):
        ...
        grad_norm = T.sqrt(sum([T.sqr(x).sum() for x in grad]))
        updates = OrderedDict()
